...
def train_classifier_simple(model, train_loader, val_loader, optimizer, device, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in train_loader:
            ...
            loss = calc_loss_batch_4_classification(input_batch, target_batch, model, device)
            ...
            if global_step % eval_freq == 0:
                train_loss, val_loss = evaluate_model_4_classification(
...
def evaluate_model_4_classification(model, train_loader, val_loader, device, eval_iter):
    model.eval()
    with torch.no_grad():
        train_loss = calc_loss_loader_4_classification(train_loader, model, device, num_batches=eval_iter)
        val_loss = calc_loss_loader_4_classification(val_loader, model, device, num_batches=eval_iter)
    model.train()
    return train_loss, val_loss

def calc_loss_loader_4_classification(data_loader, model, device, num_batches=None):
    total_loss = 0.
...
def calc_loss_batch_4_classification(input_batch, target_batch, model, device):
